...
const CONFIG = {
  ...
  LOCAL_CACHE_KEY: "mccoys-electrical-provider-list-v7"
};
...
const FIELD_ALIASES = {
  ...
  city: ["city"],
  state: ["state", "st"],
  map: ["map", "map link", "map url", "location map", "google map", "google maps", "maps", "web page", "website", "link", "url"],
...
function createResultCard(row, badgeText) {
  ...
  for (const [label, value] of fields) {
    ...
    if (label === "Outage Phone #") {
      ...
      mapLink.setAttribute("aria-label", `Open map for store ${store}`);
...
function getMapUrl(row) {
  const directMap = getValue(row, "map");
  if (directMap) return normalizeMapUrl(directMap);

  const address = getValue(row, "address");
  const site = getValue(row, "city");
  const state = getValue(row, "state") || "TX";
  const query = [address, site, state].filter(Boolean).join(", ");
...
function buildCopyText(row) {
  return [
    ...
    getMapUrl(row) ? `Map: ${getMapUrl(row)}` : "",
